...
async fn test_client_ip_extraction_integration() {
    ...
    headers.insert(
        "x-forwarded-for",
        HeaderValue::from_static("10.0.0.1, 203.0.113.2"),
    );
    ...
    headers.insert(
        "x-forwarded-for",
        HeaderValue::from_static("203.0.113.4, 192.168.1.1, 10.0.0.1"),
    );
    ...
    assert_eq!(
        ip.as_str(),
        "203.0.113.4",
        "Should use first public IP from X-Forwarded-For"
    );
    ...
    assert_eq!(
        ip.as_str(),
        "203.0.113.6",
        "Should use X-Real-IP when higher priority headers absent"
    );
    ...
    assert!(
        ip.is_empty(),
        "Should return empty ClientIp when only private IPs available"
    );
    ...
    assert!(
        ip.is_empty(),
        "Should return empty ClientIp when no IP headers present"
    );
}
...
async fn test_real_world_scenarios() {
    ...
    headers.insert(
        "x-forwarded-for",
        HeaderValue::from_static("198.51.100.1, 10.244.0.5, 192.168.1.1"),
    ); // client, pod, nginx
    ...
    assert_eq!(
        ip.as_str(),
        "198.51.100.1",
        "Cloudflare scenario should extract real client IP"
    );
    ...
    headers.insert(
        "x-forwarded-for",
        HeaderValue::from_static("203.0.113.195, 10.0.0.10"),
    ); // client, internal LB
    ...
    assert_eq!(
        ip.as_str(),
        "203.0.113.195",
        "nginx scenario should extract first public IP"
    );
    ...
    headers.insert(
        "x-forwarded-for",
        HeaderValue::from_static("192.168.1.100, 203.0.113.50, 10.0.0.1"),
    );
    ...
    assert_eq!(
        ip.as_str(),
        "203.0.113.50",
        "Should skip private IPs and find first public IP"
    );
    ...
    headers.insert(
        "x-forwarded-for",
        HeaderValue::from_static("10.128.0.5, 203.0.113.89"),
    );
    ...
    assert_eq!(
        ip.as_str(),
        "203.0.113.89",
        "Should handle mobile carrier scenarios"
    );
}
...
async fn test_security_scenarios() {
    ...
    assert_eq!(
        ip.as_str(),
        "203.0.113.100",
        "Should reject loopback IPs even from trusted headers"
    );
    ...
    headers.insert(
        "x-forwarded-for",
        HeaderValue::from_static("invalid-ip, 203.0.113.101"),
    );
    ...
    assert_eq!(
        ip.as_str(),
        "203.0.113.101",
        "Should skip invalid IPs and find valid ones"
    );
    ...
    assert!(
        ip.is_empty(),
        "Should return empty when all IPs are private/internal"
    );
}
