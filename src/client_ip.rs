...
pub fn extract_client_ip(headers: &HeaderMap) -> ClientIp {
                ...
                if *header_name == "x-forwarded-for" {
                    for ip_str in header_str.split(',') {
                        ...
                        if let Some(valid_ip) = validate_and_clean_ip(cleaned_ip) {
                            debug!(
                                extracted_ip = valid_ip,
                                source = header_name,
                                "extracted client IP from list"
                            );
                            return ClientIp::new(valid_ip);
                        }
                    }
                } else {
    ...
    warn!("no valid client IP found in headers, using empty ClientIp");
    ClientIp::empty()
}
...
mod tests {
    ...
    fn test_extract_cloudflare_ip() {
        ...
        headers.insert(
            "x-forwarded-for",
            HeaderValue::from_static("192.168.1.1, 203.0.113.1"),
        );

        let ip = extract_client_ip(&headers);
        assert_eq!(ip.as_str(), "203.0.113.1"); // Cloudflare has priority
    }
    ...
    fn test_extract_x_forwarded_for() {
        ...
        headers.insert(
            "x-forwarded-for",
            HeaderValue::from_static("203.0.113.1, 10.0.0.1, 172.16.0.1"),
        );

        let ip = extract_client_ip(&headers);
        assert_eq!(ip.as_str(), "203.0.113.1"); // First public IP
    }
    ...
    fn test_extract_x_real_ip() {
        ...
        let ip = extract_client_ip(&headers);
        assert_eq!(ip.as_str(), "203.0.113.1");
    }
    ...
    fn test_only_private_ips() {
        ...
        let ip = extract_client_ip(&headers);
        assert!(ip.is_empty()); // All IPs are private
    }

    #[test]
    fn test_validate_and_clean_ip() {
        assert_eq!(
            validate_and_clean_ip("203.0.113.1"),
    ...
    fn test_header_priority() {
        ...
        headers.insert(
            "x-forwarded-for",
            HeaderValue::from_static("203.0.113.3, 10.0.0.1"),
        );

        let ip = extract_client_ip(&headers);
        assert_eq!(ip.as_str(), "203.0.113.1"); // CF has highest priority
    }
}
